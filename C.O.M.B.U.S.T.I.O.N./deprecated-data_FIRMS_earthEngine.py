...
# Mapping of period labels to day offsets (ONLY BEFORE columns, with 4B and 5B added)
PERIODS = {
...
def get_data_for_period(day_offset, fire_date):
    ...
    # ---------- NDVI / EVI (MOD13Q1) — 16-day composite ----------
    # Use ±16 days to be more tolerant of compositing cycles
    ndvi = (ee.ImageCollection("MODIS/061/MOD13Q1")
            .filterDate(period_date.advance(-16, 'day'), period_date.advance(16, 'day'))
            .sort('system:time_start', False)
            .first())

    # ---------- LAI / FPAR (MCD15A3H) — 4-day composite ----------
    # Use ±4 days
...
def process_date(fire_date):
    ...
    def get_hotspots():
        hotspot_points = (firms.select('confidence').gt(0)
                          .reduceToVectors(
                              geometry=CERRADO, scale=1000,
        ...
        non_fire_points = (never_burned_mask
                           .stratifiedSample(
        ...
        lulc_image = (ee.ImageCollection("projects/mapbiomas-public/assets/brazil/lulc/v1")
                      .filter(ee.Filter.eq('year', lulc_year))
                      .first())
        ...
        def map_over_hotspots(feature):
            ...
            for label, offset in PERIODS.items():
                ...
                # ---- Spectral values (unchanged logic, wider windows above) ----
                period_vals = {
                    # NDVI / EVI  (MOD13Q1)
...
for i in range(int(months_diff)):
    ...
    task = ee.batch.Export.table.toDrive(
        ...
        folder='ic-fiat_firms/FIRMS_FIATs-RF',
        fileFormat='CSV',
        selectors=selectors
    )
    task.start()
