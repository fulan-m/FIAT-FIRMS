...
// ==============================================
/**
 * Processes FIRMS image to extract hotspots.
 */
function processFirmsImage(image, region, buffer_size, scale) {
  var date = ee.Date(image.get('system:time_start'));
  var hotspots = image.select('T21').gt(0);
...
// --------------------------------------------------
// MODIFIED FUNCTION: createVegetationTimeSeries
// --------------------------------------------------
/**
...
function createVegetationTimeSeries(collection, bandName, prefix, feature, scale) {
  ...
  for (var i = 0; i < CONFIG.time_series.periods; i++) {
    ['B', 'A'].forEach(function (direction) {
      ...
      // 🎯 ADDED NULL CHECK: If reduceRegion returns null, replace with -9999
      ...
      // 🎯 ADDED NULL CHECK: If the date value is null, replace with 'NA'
...
function createLaiFparTimeSeries(collection, bandName, prefix, feature, scale) {
  ...
  for (var i = 0; i < CONFIG.time_series.periods; i++) {
    ['B', 'A'].forEach(function (direction) {
      ...
      // ✅ FIX: Check if the value is null (e.g., from reduceRegion) using ee.Algorithms.IsEqual(value, null).
      // If null, set it to the designated missing value (-9999).
      var isValNull = ee.Algorithms.IsEqual(val, null);
      val = ee.Algorithms.If(isValNull, ee.Number(-9999), val);
      properties = properties.set(propPrefix, val);

      // 2. Extract and combine bitmask properties
      ...
      // ✅ FIX: Check if the date value is null (e.g., if img was null) using ee.Algorithms.IsEqual(value, null).
...
// --------------------------------------------------
// MODIFIED FUNCTION: createReflectanceTimeSeries
// --------------------------------------------------
/**
...
function processYear(startDate, endDate) {
  ...
  var hotspotPoints = firms
    .map(function (img) {
      return processFirmsImage(
        img,
        cerrado,
  ...
  var exportDescription = 'FIRMS_' + year + '_' + CONFIG.buffer_size_meters + 'km';
  
  Export.table.toDrive({
    collection: finalData,
    description: exportDescription,
    folder: 'ic-fiat_firms/FIRMS',
    fileFormat: 'CSV'
  });
