...
// =================================================
/**
 * Processes MODIS Aqua image to extract hotspots.
 */
function processAquaImage(image, region, buffer_size, scale) {
  var date = ee.Date(image.get('system:time_start'));
  
  // Extract fire confidence from FireMask bits 0-3
  var fireMask = image.select('FireMask');
  var fireConfidence = bitwiseExtract(fireMask, 0, 3);
  
  // Create hotspots: fire confidence values 7, 8, 9 (low, nominal, high confidence)
  var hotspots = fireConfidence.gte(7).and(fireConfidence.lte(9));
  ...
  var createHotspotFeature = function (feat) {
    ...
    return ee.Feature(buffer, props)
      ...
      .set('LAT', lat)
      .set('LON', lon)
      .set('MaxFRP', props.get('MaxFRP'))
      .set('FireMask', props.get('FireMask'))
      .set('QA', props.get('QA'))
...
/**
 * Extracts MODIS Aqua FireMask bitmask values for a feature
 */
function extractAquaFireMaskBitmasks(image, feature, prefix) {
  var point = ee.Geometry.Point([feature.get('LON'), feature.get('LAT')]);
  var scale = CONFIG.scales.aqua_fire;
  var properties = {};
  
  // Extract FireMask bits 0-3 (Fire confidence classes)
...
/**
 * Extracts MODIS Aqua QA bitmask values for a feature
 */
function extractAquaQaBitmasks(image, feature, prefix) {
  var point = ee.Geometry.Point([feature.get('LON'), feature.get('LAT')]);
  var scale = CONFIG.scales.aqua_fire;
  var properties = {};
  
  // Extract QA bits 0-1 (Land/water state)
  var qa_0_1 = extractBitmaskValue(image, 'QA', 0, 1, prefix, scale, point);
  properties[prefix + '_QA_0_1'] = qa_0_1;
  
  // Extract QA bit 2 (Night/day)
...
/**
 * Time series creation function for MOD13A1 vegetation indices
 */
...
function processYear(startDate, endDate) {
  ...
  var hotspotPoints = aqua
    .map(function (img) {
      return processAquaImage(
        img,
        cerrado,
        CONFIG.buffer_size_meters,
        CONFIG.scales.aqua_fire
      );
    })
    .flatten();
  ...
  var processFeature = function (feature) {
    ...
    // Use vegetation time series for MOD13A1 (with bitmask extraction)
    var ndvi = createVegetationTimeSeries(
      modisNdviEvi,
      'NDVI',
      'NDVI',
      feature,
      CONFIG.scales.ndvi_evi
    );
    var evi = createVegetationTimeSeries(
      modisNdviEvi,
      'EVI',
      'EVI',
      feature,
    ...
    // Use LAI/FPAR time series for MOD15A2H (with bitmask extraction)
    var lai = createLaiFparTimeSeries(
      modisLaiFpar,
      'Lai_500m',
      'LAI',
      feature,
    ...
    var fpar = createLaiFparTimeSeries(
      modisLaiFpar,
      'Fpar_500m',
      'FPAR',
      feature,
    ...
    // Use reflectance time series for MOD09A1 (with bitmask extraction)
    var reflectance = createReflectanceTimeSeries(
      modisReflectance,
      feature,
      CONFIG.scales.nbr
    );
  ...
  var exportDescription = 'AQUA_' + year;
  
  Export.table.toDrive({
    collection: finalData,
    description: exportDescription,
    folder: 'ic-fiat_firms/AQUA',
    fileFormat: 'CSV'
  });
...
function main() {
  var startYear = 2000;  // MODIS Aqua launched in 2002
  var endYear = 2025;
