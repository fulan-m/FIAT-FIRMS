...
// ==============================================
/**
 * Processes MODIS Terra image to extract hotspots. (Renamed from processAquaImage)
 */
function processTerraImage(image, region, buffer_size, scale) {
  var date = ee.Date(image.get('system:time_start'));
  
  var fireMask = image.select('FireMask');
  var fireConfidence = bitwiseExtract(fireMask, 0, 3);
  
  // Create hotspots: fire confidence values 7, 8, 9
...
function processYear(startDate, endDate) {
  ...
  var hotspotPoints = terra
    .map(function (img) {
      return processTerraImage(
        img,
        cerrado,
        CONFIG.buffer_size_meters,
        CONFIG.scales.terra_fire
      );
    })
    .flatten();
...
var processFeature = function (feature) {
  ...
  var exportDescription = 'TERRA_' + year;
  
  Export.table.toDrive({
    collection: finalData,
    description: exportDescription,
    folder: 'ic-fiat_firms/TERRA',
    fileFormat: 'CSV'
  });
...
function main() {
  var startYear = 2000; // Terra data is available from Feb 2000
  var endYear = 2025;
